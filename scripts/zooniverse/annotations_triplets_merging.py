...
# %% Read in manuall annotations sheet: 
master = pd.read_csv(prefix+"/data/Master_Annotations_sheet.csv")
...
scores = pd.read_csv(os.path.join(prefix+"data/"+videoname+"/_scores_triplets.csv"))
...
# %% get k frames around each frame: 
# ### testing for reconstituted filenames... 
# re.split('_frame|.jpg', scores['fname'][2])[1] # testing regex... 
# range(int(re.split('_frame|.jpg', scores['fname'][2])[1]) - k, 
#       int(re.split('_frame|.jpg', scores['fname'][2])[1]) + k+1) # plus one because ranges in python do not include last element

# for y in range(int(re.split('_frame|.jpg', scores['fname'][2])[1]) - k, int(re.split('_frame|.jpg', scores['fname'][2])[1]) + k+1): 
#     print(y)
#     print(re.split('_frame|.jpg', scores['fname'][2])[0]+"_frame"+str(y)+".jpg")
...
# %% Just convert them back to a list...  
# Get the number of focal_frame keys
num_focal_frames = len(list(temp_dict_outer.values())[0])

# Create empty lists for each focal_frame key
focal_frames = {f'focal_frame_{i}': [] for i in range(-1, num_focal_frames-1)}
...
for key, inner_dict in temp_dict_outer.items():
    for i in range(-1, num_focal_frames-1):
...
scores_expanded.to_csv(prefix+"data/"+videoname+"/scores_expanded.csv")
...
scores_merged.to_csv(prefix+"data/"+videoname+"/scores_merged.csv")
...
manifest.to_csv(prefix+"data/"+videoname+"/manifest.csv")
